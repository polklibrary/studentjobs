...
class JobAppView(BaseView):
    ...
    def jobapp(self):
        ...
        if 'applicant.submit' in self.request.params:
            ...
            local_address = self.request.params.get('applicant.local_address','')
            home_address = self.request.params.get('applicant.home_address','')
            ...
            service_experience = self.request.params.get('applicant.service_experience','')
            tech_experience = self.request.params.get('applicant.tech_experience','')
            library_assets = self.request.params.get('applicant.library_assets','')
            reference_one_name = self.request.params.get('reference.one.name','')
            reference_one_phone = self.request.params.get('reference.one.phone','')
            reference_one_acquainted = self.request.params.get('reference.one.acquainted','')
            reference_two_name = self.request.params.get('reference.two.name','')
            reference_two_phone = self.request.params.get('reference.two.phone','')
            reference_two_acquainted = self.request.params.get('reference.two.acquainted','')
            employer_one_name = self.request.params.get('employer.one.name','')
            employer_one_phone = self.request.params.get('employer.one.phone','')
            employer_one_leave = self.request.params.get('employer.one.leave','')
            employer_two_name = self.request.params.get('employer.two.name','')
            employer_two_phone = self.request.params.get('employer.two.phone','')
            employer_two_leave = self.request.params.get('employer.two.leave','')
